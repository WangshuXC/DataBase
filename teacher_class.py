...
class Teacher:
    def __init__(self,TNO):
        ...
        self.course_label = Label(self.root, text="请选择课程名：",bg="saddlebrown",fg="white",font=("黑体", 12))
        self.student_label = Label(self.root, text="已选修此课程的学生：",bg="saddlebrown",fg="white",font=("黑体", 12))

        self.lb = Listbox(self.root, width=20, height=10)
        self.student_text = Text(self.root, width=20, height=14)

        Label(self.root,text="学号：",bg="saddlebrown",fg="white",font=("黑体", 12)).grid(row=3,column=3)
        Label(self.root, text="成绩：",bg="saddlebrown",fg="white",font=("黑体", 12)).grid(row=5, column=3)

        self.student_id_entry = Entry(self.root, text="学号",width=5)
        self.student_grape_entry = Entry(self.root, text="成绩",width=5)

        self.student_button = Button(self.root, text="查询", command=self.find_score,bg="cadetblue",fg="white",font=("黑体", 12))
        self.score_button =Button(self.root, text="输入成绩",bg="cadetblue",fg="white",font=("黑体", 12),command=self.change_score)
