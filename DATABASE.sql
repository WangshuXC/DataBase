...
INSERT INTO `databasegrade` (`SNO`, `GRADE`)
VALUES ('S2110951', 100);
CREATE DEFINER = `root` @`localhost` PROCEDURE `updatascore`(
    IN `New_SNO` char(10),
    IN `New_CNO` char(10),
    IN `New_GRADE` tinyint
) BEGIN
UPDATE sc
SET GRADE = New_GRADE
WHERE SNO = New_SNO
...
END IF;
END;
CREATE TRIGGER `UpdataDatabase`
AFTER
UPDATE ON `sc` FOR EACH ROW BEGIN IF(new.CNO = 'C5') THEN
UPDATE databasegrade
SET grade = new.grade
WHERE SNO = new.SNO;
END IF;
END;
CREATE TRIGGER `AddDatabase` BEFORE
INSERT ON `sc` FOR EACH ROW BEGIN IF(new.CNO = 'C5') THEN
INSERT INTO databasegrade
VALUES(new.SNO, 0);
END IF;
END;
